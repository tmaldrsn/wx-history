...
if __name__ == "__main__":
    ...
    cur = con.cursor()
    cur.execute(
        """create table if not exists station('id', 'state', 'name', 'latitude', 'longitude', constraint unq unique (id))""")
    ...
    for station in station_table:
        station_data.append([station.station_id.string, station.state.string,
                             station.station_name.string, station.latitude.string, station.longitude.string])

    # Send the data to the database
    cur.executemany(
        """insert or replace into station values (?, ?, ?, ?, ?)""", station_data)
