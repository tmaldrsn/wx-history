...
def main():
    ...
    for station in stations:
        ...
        try:
            cur.execute(
                f"""create table if not exists {station} ({','.join('"{0}"'.format(w) for w in forecast_elements)}, constraint unq unique (date, time))""")
            # logger.debug(f"Created table for {station}")
        ...
        try:
            req = urllib.request.urlopen(
                f"https://w1.weather.gov/data/obhistory/{station}.html", timeout=TIMEOUT)
        ...
        data_rows = [[datetime.datetime.today().strftime(f"%m/{row.find_all('td')[0].string}/%Y {row.find_all('td')[1].string}")] + [
            element.string for element in row.find_all('td')[1:]] for row in forecast_rows]
        for i in range(1, len(data_rows)):
            init_date = datetime.datetime.strptime(
                data_rows[i-1][0], "%m/%d/%Y %H:%M")
            next_date = datetime.datetime.strptime(
                data_rows[i][0], "%m/%d/%Y %H:%M")
            if init_date != next_date:
                data_rows[i][0] = datetime.datetime.strftime(
                    init_date - datetime.timedelta(hours=1), "%m/%d/%Y %H:%M")
        ...
        cur.executemany(
            f"""insert or replace into {station} values ({','.join(['?' for i in range(len(forecast_elements))])})""", data_rows)
        logger.debug(
    ...
    con.commit()
    logger.info(
        "Initial observations update complete, now checking for timed out stations...")
...
#	print(f"{len(timed_out)} stations timed out during first round: {timed_out}")
    ...
    while len(timed_out) > 0:
        ...
        for i in range(len(timed_out)):
            ...
            try:
                req = urllib.request.urlopen(
                    f"https://w1.weather.gov/data/obhistory/{station}.html", timeout=5)
                soup = BeautifulSoup(req, 'html.parser')
            except (OSError, URLError):
                logger.warning(
                    f"Station {station} request timed out again.. this time in round {current_round}")
#				print(f"Station {station} request timed out again.. this time in round {current_round}")
            ...
            for j in range(1, len(data_rows)):
                init_date = datetime.datetime.strptime(
                    data_rows[j-1][0], "%m/%d/%Y %H:%M")
                next_date = datetime.datetime.strptime(
                    data_rows[j][0], "%m/%d/%Y %H:%M")
                if init_date != next_date:
                    data_rows[j][0] = datetime.datetime.strftime(
                        init_date - datetime.timedelta(hours=1), "%m/%d/%Y %H:%M")
        ...
        if len(timed_out) != 0:
            #    print(
            #        f"Round {current_round} complete. Stations left: {len(timed_out)}: {timed_out}")
            logger.info(
                f"Round {current_round} of re-requesting timed out URLs complete. {len(timed_out)} stations still need updated.")
        else:
            #    print(
